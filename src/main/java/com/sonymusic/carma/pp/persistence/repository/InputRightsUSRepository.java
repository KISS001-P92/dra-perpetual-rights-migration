...
public interface InputRightsUSRepository {
	...
	@Transactional
	@Modifying
	@Query(value = "UPDATE dra_perpetual_rights_migration_input_us SET status_flag = 'A', mod_stamp = NOW(), mod_user = 99999998 WHERE  id = :id", nativeQuery = true)
	void updateProcessed(Integer id);

	@Transactional
	@Modifying
	@Query(value = "UPDATE dra_perpetual_rights_migration_input_us SET status_flag = 'D', mod_stamp = NOW(), mod_user = 99999998 WHERE status_flag='A' AND id = :id", nativeQuery = true)
